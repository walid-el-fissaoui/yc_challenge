...
@section('content')
    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
        ...
        @enderror
        <input type="number" step="0.01" name="price" id="priceField" placeholder="Price"
            class="form-control mb-3 @error('price') is-invalid @enderror" value="{{ old('price') }}">
        @error('price')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <input type="file" accept="image/*" name="image" id="imageField" placeholder="Image"
            class="form-control mb-3 @error('image') is-invalid @enderror">
        @error('image')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <textarea name="description" id="descriptionField" placeholder="Description"
            class="form-control mb-3 @error('description') is-invalid @enderror" value="{{ old('description') }}" cols="30"
            rows="5"></textarea>
        @error('description')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
